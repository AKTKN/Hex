...
def steane_error_correction(
    ...
    plus_state_prep_data_qubit = no_measurement_module(plus_state_prep_circuit_noiseless, first_block_support)
    zero_state_prep_data_qubit = no_measurement_module(zero_state_prep_circuit_noiseless, first_block_support)
    ...
    for round_number in range(rounds_of_steane_error_correction):
        ...
        modules_for_steane_error_correction.append(no_measurement_module(zero_state_prep_circuit_noiseless, second_block_support))
        ...
        #     matchable=True,
        # ))
        modules_for_steane_error_correction.append(generate_transversal_cnot_module(physical_error, second_block_support, first_block_support))
        ...
        modules_for_steane_error_correction.append(no_measurement_module(plus_state_prep_circuit_noiseless, second_block_support))
        # modules_for_steane_error_correction.append(generate_state_prep_module(
        #     code,
        #     distance,
        #     "x",
        ...
        #     matchable=True,
        # ))
        modules_for_steane_error_correction.append(generate_transversal_cnot_module(physical_error, first_block_support, second_block_support))
        if include_correction:
            print("include")
...
if __name__ == "__main__":
    physical_error_range = np.linspace(0.01, 0.08, 5)
    distance_range = [3, 5]
    num_shots = 50_000
    pauli = "x"
    ...
    threshold_plot_from_function(
        ...
        num_shots,
        [({"distance": distance, "pauli": pauli, "repetitions": rep, "include_correction": correction},
          f"Distance = {distance}, Repetitions = {rep}, Correction = {correction}")
         for distance in distance_range
         for rep in [8]
         for correction in [True, False]
         ],
        title = f"Steane EC Memory Experiment, Measuring logical {'Z' if (pauli.lower() == 'z') else 'X'}",
