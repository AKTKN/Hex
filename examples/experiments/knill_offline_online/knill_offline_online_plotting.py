...
def main():
    ...
    for (code, offline_decoder, online_decoder, num_teleportations), group in summary_df.groupby(["code", "online_decoder", "offline_decoder", "num_teleportations"]):
        ...
        if len(pauli) != 1:
            ...
            # title = f"Knill EC Memory Experiment, Measuring logical {'Z' if (pauli[0].lower() == 'z') else 'X'}"
...
#     else:
#         z = 1.96
#         summary_df["logical_errors_prime"] = summary_df["logical_errors"] + z
#         summary_df["samples_performed_prime"] = summary_df["samples_performed"] + z**2
#         summary_df["logical_error_rate_prime"] = summary_df["logical_errors_prime"] / summary_df["samples_performed_prime"]
#         summary_df["error_bar"] = z*np.sqrt((summary_df["logical_error_rate_prime"] * (1 - summary_df["logical_error_rate_prime"])) / summary_df["samples_performed_prime"]
#                                           )

#         plotting_df["physical error"] = summary_df["physical_error_rate"]
#         plotting_df["logical error"] = summary_df["logical_error_rate_prime"]
#         plotting_df["logical error interval above"] = summary_df["error_bar"]
#         plotting_df["logical error interval below"] = summary_df["error_bar"]
#         plotting_df["label"] = summary_df.apply(label_function, axis=1)

#         repetitions = summary_df["repetitions"][0] # assuming their is only one number of repetitions

#         # Generate CSV for plotting
#         plotting_df.to_csv(f"plotting_data.csv", index=False)
#         # title = f"Knill EC Memory Experiment, Measuring logical {'Z' if (pauli[0].lower() == 'z') else 'X'}"
#         title = f"Surface Code, {repetitions} Repetitions of Knill"
#         generate_threshold_plot(
#             "plotting_data.csv",
#             title,
#             style="Quantum",
